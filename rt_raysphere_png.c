...
float *lm_rt_primary_rays( int width, int height ) {
  ...
  for( y=0; y<height; y++ ) {
    for( x=0; x<width; x++ ) {
...
#ifdef MP
      nx = mpfr_get_flt( n.x, MPFR_RNDN );
      ny = mpfr_get_flt( n.y, MPFR_RNDN );
      nz = mpfr_get_flt( n.z, MPFR_RNDN );
#else
      nx = n.x;
      ny = n.y;
      nz = n.z;
      ...
      buffer[ y * width + x ] = (hit == 1) ? sqrt(nx*nz + ny*nz) : 0.0f;
    }
  }
  return buffer;
